;(function($){
	...
	var allobj = {
		...
		init:function(){
			...
			$(".table tbody").on("mouseover", "td", function(){
				var $t = $(this);
				var $dom = $t.find('.table_tip');
				if ($dom.length<=0){
					$dom = $("<div class='table_tip'>"+$t.text()+"</div>");
					$t.append($dom);
				}
				$dom.css({
					top: $t.offset().top - $("body").scrollTop() - 25 + "px"	
				});
				$dom.stop().fadeIn(200);
			}).on("mouseout","td", function(){
				var $dom = $(this).find(".table_tip");
				if ($dom.length > 0) {
					$dom.stop().fadeOut(200);
				}
			});
			/*============================
			...
			$(".all_markbtn").on("click",function(){
				...
				// 如果指定了手机input,则此input中的值必须是正确手机号才会触发效果
				if($t.data('phone')) {
					var v = $("#"+$t.data('phone')).val();
					var reg = /^[1][358][0-9]{9}$/g;
					if(!reg.test(v)) {	// 未通过验证返回false
						allobj.toast("手机号不正确");
						return false;
					}
				}
		...
		*/
		sToast:function(a,b){
			var $s = $(".all_stoast");
			if($s.length<=0){
				$s = $('<div class="all_stoast"><div><img src="../img/loading.gif"><div class="t"></div></div></div>');
				$("body").append($s);
			}
		...
		*/
		...
		//lo_id:验证身份证格式是否合法
		//lo_number:验证是否为指定范围的整数，需在input上写data-number="1000-2000"这样
		
		//.lodiv(input组合容器,里面应该包括input,正确的提示，错误的提示)
		//.loisok(正确提示的容器)
		//.loisno(错误提示的容器)
		//.lochecked(对某个多选框或单选框，必须勾选才通过验证)
		locheckInit:function(){
			...
			$lo.find(".lo_username").on("keyup blur",function(){louser(this);});
			...
			$lo.find(".lo_id").on("keyup blur",function(){loid(this);});
			$lo.find(".lo_number").on("keyup blur",function(){lonumber(this);});
		...
		lobegincheck:function(t){
			...
			$p.find(".lo_int,.lo_phone,.lo_nonull,.lo_pwd,.lo_pwdr,.lo_email,.lo_yzm,.lo_id,.lo_number,.lo_checked").trigger("keyup");
	...
	function lophone(t){
		...
		var isok = !!reg.test(v);
		loshow(t,isok);
		$.post("checkPhone",{phone:v},function(result){
			var isok1 = result.errNum;
	...
	function louser(t){
		...
		var isok = !!(v.replace(/(^\s*)|(\s*$)/g, ""));
		loshow(t,isok);
		$.post("checkUserName",{user_name:v},function(result){
			var isok = result.errNum;
			var msg = '*'+result.errMsg;
			loshow(t,isok,msg);
			return isok;
		});
	...
	function lopwd(t){
		...
		//关联触发密码repeat验证
		if($t.data('pwdr')) {
			$t.parents(".locheck").find(".lo_pwdr[data-pwdr="+$t.data("pwdr")+"]").trigger("keyup");
		}
		loshow(t,isok);
		return isok;
	}
	...
	function lopwdr(t){
		var $t = $(t);
		var $t2 = $t.parents(".locheck").find(".lo_pwd[data-pwdr="+$t.data("pwdr")+"]").eq(0);
		var v1 = $(t).val();
		var v2 = $t2.val();
		if(v1 && v1 == v2){
			loshow(t,true);
	...
	function lonumber(t){
		...
		if (num !== 0 && !num) {
			...
			isOk = false;
		} else if (num < parseInt(fw[0]) || num > parseInt(fw[1])) {
			isOk = false;	
		} else {
			isOk = true;	
		}
